 ...
 * @version $Id: ProjectDialog.js 20938 2015-10-22 09:26:31Z wsmits $
	...
	show : function (config) {
		...
		if(!config.project_id)
		{
			...
			}else
			{
				...
				if(GO.projects2.useNameTemplate == true || GO.projects2.useNameTemplate == 1)
				{
					if(!config.values.name && GO.projects2.nameTemplate!='')
						config.values.name=GO.projects2.nameTemplate+' ';
				}
			}
		}
                ...
                this.formPanel.load({
                    ...
                    success:function(form, action)
                    {
						...
						if(this.duplicate_id) {
							if(GO.projects2.useNameTemplate)
								this.nameField.setValue(GO.projects2.nameTemplate);
							else
								this.nameField.setValue(this.nameField.getValue() + ' ('+GO.projects2.lang['copy']+')');
						}
...
//			
	...
	buildForm : function () {
		...
		leftCol.add(this.parentField = new Ext.form.DisplayField({
			name: 'parent_project_path',
			anchor: '100%',
			allowBlank:false,
