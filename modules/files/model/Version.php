...
class Version extends \GO\Base\Db\ActiveRecord {
	...
	protected function afterSave($wasNew) {
		...
		$this->file->folder->quotaUser->calculatedDiskUsage($this->size_bytes)->save(true); //user quota
	...
	protected function beforeDelete() {
		...
		if($file->delete() && $this->file->folder->quotaUser) {
			$this->file->folder->quotaUser->calculatedDiskUsage(0 - $this->size_bytes)->save(true); //user quota
			\GO::config()->save_setting("file_storage_usage", \GO::config()->get_setting('file_storage_usage')-$this->size_bytes);
		}
