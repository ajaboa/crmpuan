			...
			GO.tasks.tasksObservable.on('save',function(dialog, task_id, loadedStore){
				if(dialog.showConfig) {
					...
					if (emailMessage = dialog.showConfig.emailMessage) {
						...
						var params = {
							account_id: emailMessage.account_id,
							mailbox: emailMessage.mailbox,
							links: Ext.encode([{
								model_name:'GO\\Tasks\\Model\\Task',
								model_id: task_id
							}]),
							uids: Ext.encode([emailMessage.uid]),
							total: 1
						};
					...
					if(!this.linksDialog)
					{
						this.linksDialog = new GO.dialog.LinksDialog({
							...
							sendLinkRequest : function(tolinks, to_folder_id){
								...
								var linkRequest = function(newUids){

									if(!newUids)
									{
										params.total=uids.length;
									}else
									{
										params.uids=Ext.encode(newUids);
									}
										...
										callback: function(options, success, response)
										{
											...
											}else
											{
												...
												if(!responseParams.success)
												{
													alert(responseParams.feedback);
													Ext.MessageBox.hide();
												}else if(responseParams.uids && responseParams.uids.length>0)
												{
													Ext.MessageBox.updateProgress(responseParams.progress, (responseParams.progress*100)+'%', '');

													linkRequest.call(this, responseParams.uids);
												}else
												{
