...
class FileController extends \GO\Base\Controller\AbstractController {
	...
	protected function actionImport($params){
		...
		if ($this->client->getAccessToken()) {
			...
			$file = $this->service->files->get(\GO::session()->values['googledrive']['editing'][$params[id]]['gd_id']);

			$goFile = \GO\Files\Model\File::model()->findByPk(\GO::session()->values['googledrive']['editing'][$params[id]]['go_file_id']);
			...
			if ($downloadUrl) {
				...
				if ($httpRequest->getResponseHttpCode() == 200) {
					...
					$this->service->files->delete(\GO::session()->values['googledrive']['editing'][$params[id]]['gd_id']);
					
					unset(\GO::session()->values['googledrive']['editing'][$params[id]]);
					
					$response['success']=true;
				}  else {
					throw new Exception("Got HTTP response code ".$httpRequest->getResponseHttpCode()." from Google");
				}
			}else
			{
	...
	protected function actionEdit($params) {
		...
		/* ### DELETING RENAME FOR OLD EXTENSION FILES
		...
		*/
		try{
			if ($this->client->getAccessToken()) {
				...
				\GO::session()->values['googledrive']['editing'][$goFile->id]=array('go_file_id'=>$goFile->id, 'gd_id'=>$createdFile->id);
				...
				GO::session()->values['googledrive'][id]['edit_id_after_auth']=$params['id'];
	...
	protected function actionDelete($params) {
		if(\GO::session()->values['googledrive']['editing'][$params[id]]){
			$this->service->files->delete(\GO::session()->values['googledrive']['editing'][$params[id]]['gd_id']);
			unset(\GO::session()->values['googledrive']['editing'][$params[id]]);
			$response['success']=true;
		}
	...
	protected function actionFilename($params) {
		if($goFile = \GO\Files\Model\File::model()->findByPk($params['id'])){
			$response['results'] = $goFile->name;
			$response['success'] = true;
			//$store = new \GO\Base\Data\ArrayStore();
			//$store->addRecord(array('name'=>$goFile->name));		
