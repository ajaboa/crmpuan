...
//		}
	...
	private function _checkPageBreak(){
		if($this->_pageBreak){
			...
			$this->AddPage();
			if (!$this->_template['logo_only_first_page']){
	
				// If the logo height has the same height as the page then set the correct Y value.
				// Needed because if we also add the $this->_template['logo_height'] to the Y value then a blank page will be added.
				if($this->h === $this->_template['logo_height']){
					$this->setY($this->_template['margin_top']+$this->_template['logo_top']);
				} else {
					$this->setY($this->_template['logo_top']+$this->_template['margin_top']+$this->_template['logo_height']);
				}
